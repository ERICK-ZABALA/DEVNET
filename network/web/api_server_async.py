...
def get_devices():
    ...
    try:
        ...
        if not last_device.id:
            device_json = {"device": "http://172.16.1.123:5000/devices/",
                           "message": "Not exist data record"}
        ...
        device_urls = [
            f"http://172.16.1.123:5000/devices/{i}" for i in range(1, last_id + 1)]
...
def update_device(id):
    ...
    existing_device.hostname = updated_data.get(
        'hostname', existing_device.hostname)
    existing_device.loopback = updated_data.get(
        'loopback', existing_device.loopback)
    existing_device.mgmt_ip = updated_data.get(
        'mgmt_ip', existing_device.mgmt_ip)
    ...
    def wrapped(*args, **kwargs):
        ...
        def task():
            ...
            try:
                ...
                background_tasks[id] = make_response(response)
            except Exception as e:
                # The wrapped function raised an exception, return a 500
                # response
                background_tasks[id] = make_response(str(e), 500)
        ...
        return jsonify({"message": "Background task started", "task_id": id}), 202, {'Location': url_for('get_task_status', id=id)}
...
def get_task_status(id):
    ...
    if rv is None:
        return jsonify({"error": "Error status device", "details": str(rv)}), 500
